...
const SessionsList: React.FC<SessionsListProps> = ({
    ...
    const pendingSessions = useMemo(() => {
        return filteredSessions.filter(session => session.status === SessionStatus.PENDING);
    }, [filteredSessions]);

    const activeSessions = useMemo(() => {
        return filteredSessions.filter(session => session.status === SessionStatus.ACCEPTED);
    }, [filteredSessions]);

    const pastSessions = useMemo(() => {
        return filteredSessions.filter(
            session => session.status === SessionStatus.COMPLETED || session.status === SessionStatus.DECLINED
        );
    }, [filteredSessions]);
