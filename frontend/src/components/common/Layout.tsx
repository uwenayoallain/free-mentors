...
const Layout: React.FC<LayoutProps> = ({ children, maxWidth = 'lg' }) => {
    ...
    const successMessage = useSelector((state: RootState) => state.sessions.successMessage);
    ...
    const errorMessage = authError?.message || mentorError?.message || sessionError?.message;
            ...
            >
                <Container maxWidth="lg">
                    <Box sx={ { textAlign: 'center' } }>
                        Free Mentors &copy; { new Date().getFullYear() } - A social initiative for mentorship
                    </Box>
                </Container>
            </Box>
        </Box>
